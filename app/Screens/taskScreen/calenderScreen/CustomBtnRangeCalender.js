  ...
  const setRange = (rangeType) => {
    ...
    switch (rangeType) {
      ...
      case 'Last quarter':
        ...
        const startOfLastQuarter = new Date(today.getFullYear(), quarterStartMonth, 1);
        const endOfLastQuarter = new Date(today.getFullYear(), quarterStartMonth + 3, 0);
  ...
  return (
    <div className="flex flex-col md:flex-row gap-2 md:gap-8 xl:gap-11 rounded-lg shadow-xl w-fit py-2 px-2 md:py-4 md:px-3 bg-white custom-date-range-picker">
      ...
      <div className="flex max-md:mt-7 max-md:flex-wrap md:flex-col">
        {['Today', 'Yesterday', 'Last week', 'Last month', 'Last quarter'].map((range) => (
          <button
            key={range}
        ...
        <button onClick={resetDates} className="mt-auto py-2 px-3 text-left text-brightBlue text-[13px] md:text-base font-MetRegular leading-[125%]">
          Reset
        </button>
      </div>
